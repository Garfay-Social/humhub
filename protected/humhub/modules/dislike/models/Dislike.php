...
namespace humhub\modules\dislike\models;

use Yii;
use humhub\modules\content\components\ContentAddonActiveRecord;
use humhub\modules\content\interfaces\ContentOwner;
use humhub\modules\dislike\notifications\NewDislike;
 ...
 * @since 0.5
 */
class Dislike extends ContentAddonActiveRecord
{
     ...
     */
    public static function GetDislikes($objectModel, $objectId)
    {
        $cacheId = "dislikes_" . $objectModel . "_" . $objectId;
        $cacheValue = Yii::$app->cache->get($cacheId);

        if ($cacheValue === false) {
            $newCacheValue = Dislike::findAll(['object_model' => $objectModel, 'object_id' => $objectId]);
    ...
    {
        Yii::$app->cache->delete('dislikes_' . $this->object_model . "_" . $this->object_id);

        if ($insert) {
            \humhub\modules\dislike\activities\Disliked::instance()->about($this)->save();
            ...
            if ($this->getSource() instanceof ContentOwner && $this->getSource()->content->createdBy !== null) {
                ...
                NewDislike::instance()->from(Yii::$app->user->getIdentity())->about($this)->send($target);
            }
        }
